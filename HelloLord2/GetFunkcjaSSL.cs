...
{
    public static string GetFunkcjaSSL(string front_domain, string real_domain, string url)
    {
        try
        {
            ...
            Task<string> task = GetRequestSSL(front_domain, url, real_domain);
            task.Wait();
#if DEBUG
    ...
    static async Task<string> GetRequestSSL(string front_domain, string url, string real_domain)
    {
