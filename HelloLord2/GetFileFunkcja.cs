...
{
    public static void GetFileFunkcja(string front_domain, string real_domain, string filename, string filename_dest)
    {
        try
        {
            ...
            Task<string> task_get = GetFile(front_domain, filename, real_domain, filename_dest);
            task_get.Wait();
#if DEBUG
    ...
    static async Task<string> GetFile(string front_domain, string filename, string real_domain, string filename_dest)
    {
        ...
        try
        {
            using (var client = new HttpClient())
            {
                ...
                using (var request = new HttpRequestMessage(HttpMethod.Get, "http://" + front_domain + "/" + filename))
                {
                    using (
